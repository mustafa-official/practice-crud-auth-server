require('dotenv').config()
const express = require('express');
const { MongoClient, ServerApiVersion, ObjectId } = require('mongodb');
const cors = require('cors');
const app = express();
const port = process.env.PORT || 5000;

//practiceCrudAuth
// z3b48DtbPN4JaFlL

//middleware
app.use(cors());
app.use(express.json());

const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.elzgrcu.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0`;
// Create a MongoClient with a MongoClientOptions object to set the Stable API version
const client = new MongoClient(uri, {
    serverApi: {
        version: ServerApiVersion.v1,
        strict: true,
        deprecationErrors: true,
    }
});

async function run() {
    try {
        // Connect the client to the server	(optional starting in v4.7)
        // await client.connect();
        const practiceCollection = client.db('practiceDB').collection('user');
        //for authUser
        const authCollection = client.db('practiceDB').collection('authUser')

        //read multiple user
        app.get('/user', async (req, res) => {
            const cursor = practiceCollection.find();
            const result = await cursor.toArray();
            res.send(result)
        })

        //specific user
        app.get('/user/:id', async (req, res) => {
            const id = req.params.id;
            const query = { _id: new ObjectId(id) };
            const result = await practiceCollection.findOne(query);
            res.send(result);
        })

        //update
        app.put('/user/:id', async (req, res) => {
            const id = req.params.id;
            const user = req.body;
            const filter = { _id: new ObjectId(id) };
            const options = { upsert: true };
            const updateInfo = {
                $set: {
                    name: user.name,
                    photo: user.photo,
                    quantity: user.quantity,
                    taste: user.taste,
                    supplier: user.supplier
                }
            }
            const result = await practiceCollection.updateOne(filter, updateInfo, options);
            res.send(result);

        })

        //delete a user
        app.delete('/user/:id', async (req, res) => {
            const id = req.params.id;
            const query = { _id: new ObjectId(id) };
            const result = await practiceCollection.deleteOne(query);
            res.send(result);
        })

        //create user
        app.post('/user', async (req, res) => {
            const user = req.body;
            const result = await practiceCollection.insertOne(user);
            res.send(result);
        })


        //authentication info
        app.get('/authUser', async (req, res) => {
            const cursor = authCollection.find();
            const result = await cursor.toArray();
            res.send(result)
        })
        app.post('/authUser', async (req, res) => {
            const user = req.body;
            const result = await authCollection.insertOne(user);
            res.send(result)
        })




        // Send a ping to confirm a successful connection
        // await client.db("admin").command({ ping: 1 });
        console.log("Pinged your deployment. You successfully connected to MongoDB!");
    } finally {
        // Ensures that the client will close when you finish/error
        // await client.close();
    }
}
run().catch(console.dir);


app.get('/', (req, res) => {
    res.send('CRUD auth is running')
})
app.listen(port, () => {
    console.log(`Currently running port on ${port}`);
})